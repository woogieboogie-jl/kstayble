...
Trust boundary: on-chain = event hashes only (6 events); off-chain (device/issuer) = passport no., PII, payment/VC originals. PIPA-aligned data minimization; demo data is mock so no real PII, and audited PIPA compliance is a finals/post task. The exact ledger/consensus (e.g. Besu) follows OmniOne's spec / TBC — not independently verified.
...
Issuer verification, revocation, replay/MITM/phishing/forgery defenses = standard VC/FIDO design, NOT live in the demo — wired at finals via Open DID. Double-spend: voucher lifecycle anchored as hashes for an auditable single-use record (design); stablecoin payment finality / settlement depend on the finals payment rail and are not yet defined — the demo (localStorage balance mock) does not guarantee it.
